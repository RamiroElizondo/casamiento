...
export const INVITES = {
  ...
  '19a464a3-47dd-4210-b9d9-f2ea0f9861de': 'fiestaPago',
};

export const WHATSAPP_NUMBER = '5492645514237'; // Gonzalo
...
export function whatsappUrl(type, details = {}) {
  const cfg = INVITE_TYPES[type];
  const label = cfg.rsvpLabel;
  const name = (details.name || '').trim();
  // Corazon como escape unicode en vez del glifo crudo: el glifo se
  // corrompe facil al sincronizar el archivo entre editores u OneDrive
  // con una codificacion distinta a UTF-8, y termina llegando roto al
  // mensaje de WhatsApp. El escape es ASCII puro, no se rompe.
  const heart = '\u2764\uFE0F';
  let msg = `¡Hola! Quiero confirmar mi asistencia a ${label} del casamiento de Yamila y Gonzalo`;
  if (name) {
    msg += `\n\n*Nombre:* ${name}`;
  }
  if (cfg.payment) {
    msg += `\n\n*Me comprometo a abonar antes del 10 de octubre*`;
  }
  return `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(msg)}`;
}

export function whatsappDeclineUrl(type) {
  const label = INVITE_TYPES[type].rsvpLabel;
  const msg = `Hola, lamentablemente no voy a poder asistir a ${label} del casamiento de Yamila y Gonzalo. ¡Gracias por la invitación!`;
  return `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(msg)}`;
}
